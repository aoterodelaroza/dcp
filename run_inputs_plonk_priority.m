...
function sout = run_inputs_plonk_priority(ilist,xdmcoef=[],xdmfun="",extrad3="")
  ...
  %% back to the caller. Unlike run_inputs_plonk, the new jobs
  %% are added at the beginning of the private queue.
  
  global verbose ferr
  
  ## Debug
  if (ferr > 0) 
    fprintf(ferr,"# Start run_inputs_plonk_priority - %s\n",strtrim(ctime(time())));
    fflush(ferr);
  endif
  ...
  if (ferr > 0) 
    fprintf(ferr,"# Prepending scripts to the plonk queue - %s\n",strtrim(ctime(time())));
    fflush(ferr);
  endif
  ...
  ## Append the temporary file to the job file
  [s1 out] = system(sprintf("cat %s >> %s",jobfile,tempfile));
  [s2 out] = system(sprintf("mv %s %s",tempfile,jobfile));
  [s3 out] = system(sprintf("rm -f %s",tempfile));
  ...
  ## Debug
  if (ferr > 0) 
    fprintf(ferr,"# End run_inputs_plonk_priority - %s\n",strtrim(ctime(time())));
    fflush(ferr);
  endif
