...
function s = run_inputs_nint_gino(ilist,cont=0,xdm=[],xdmfun="")
  ...
  [s out] = system(sprintf("find . -name '%s_*.tar.bz2' | xargs mv -t /home/dilabiog/alberto/run/%s",prefix,dirname));
  [s out] = system(sprintf("find . -name '%s_*.sub' | xargs mv -t /home/dilabiog/alberto/run/%s",prefix,dirname));
  ...
  [s out] = system(sprintf("find /home/dilabiog/alberto/run/%s/ -name '*.tar.bz2' | xargs mv -t .",dirname));
  for i = 1:npack
